...
const source = fs.readFileSync(path.join(root, "src", "data", "entities.ts"), "utf8");
...
const productRecords = entities.map((entity) => ({
  ...
  visibility: entity.disposition === "review-required" ? "review" : "public",
  featured: Boolean(entity.featured),
  canonical_url: `https://groupsum.xyz/products/records/${entity.slug}`,
...
const target = path.join(root, "catalog", "content", "records.json");
fs.writeFileSync(
  target,
