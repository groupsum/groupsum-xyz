...
function SelectFilter({ label, value, options, onChange }: { label: string; value: string; options: string[]; onChange: (value: string) => void }) {
  ...
  return <label>
    ...
    <select value={value} onChange={(event) => onChange(event.target.value)} className="w-full rounded-lg border border-[var(--color-border-soft)] bg-white px-3 py-2 font-mono text-xs text-ink focus:border-accent focus:outline-none focus:ring-2 focus:ring-accent">
...
export function ExplorerFilterToolbar({ filters, onChange, owners = [], ecosystems = [], publications = [], resourceTypes = [], sortOptions, total, statusDetail = "Filtered projection" }: { filters: ExplorerFilters; onChange: (next: ExplorerFilters) => void; owners?: string[]; ecosystems?: string[]; publications?: string[]; resourceTypes?: string[]; sortOptions: Option[]; total: number; statusDetail?: string }) {
  ...
  return <div className="space-y-3 py-4">
    <div className="flex flex-wrap items-center gap-3 rounded-xl border border-[var(--color-border-soft)] bg-canvas p-3">
      <label className="relative min-w-[200px] flex-1">
        ...
        <input value={filters.search} onChange={(event) => update({ search: event.target.value })} placeholder="Search by name, summary, owner, language..." className="w-full rounded-lg border border-[var(--color-border-soft)] bg-white py-2 pl-9 pr-8 font-mono text-xs text-ink placeholder:text-[#A3A8A2] focus:border-accent focus:outline-none focus:ring-2 focus:ring-accent" />
      ...
      <SelectFilter label="All Owners / Orgs" value={filters.owner} options={owners} onChange={(value) => update({ owner: value })} />
      ...
      <label className="relative ml-auto flex items-center gap-1">
        <ArrowUpDown className="h-3.5 w-3.5 text-ink-muted" aria-hidden="true" />
        <span className="sr-only">Sort records</span>
        <select value={filters.sort} onChange={(event) => update({ sort: event.target.value })} className="rounded-lg border border-[var(--color-border-soft)] bg-white px-3 py-2 font-mono text-xs text-ink focus:border-accent focus:outline-none focus:ring-2 focus:ring-accent">
