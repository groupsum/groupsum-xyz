  ...
  return <dl aria-label={label} className="grid grid-cols-2 gap-3 pt-2 sm:grid-cols-3 lg:grid-cols-5">
    {items.map((item) => {
      ...
      return <div key={item.label} className="flex min-w-0 flex-col justify-between space-y-1 rounded-xl border border-[var(--color-border-soft)] bg-white p-3 shadow-sm transition-colors hover:border-[#B5B0A6]">
        ...
        <dd className="font-mono text-xl font-bold tabular-nums tracking-tight text-ink">{typeof item.value === "number" ? item.value.toLocaleString() : item.value}</dd>
...
export function MemberRowCard({
  ...
  return <article className="catalog-density-card group relative flex cursor-pointer flex-wrap items-center gap-x-4 gap-y-4 rounded-xl border border-[var(--color-border-soft)] bg-white p-5 shadow-sm transition-all duration-200 hover:border-accent-hover hover:bg-canvas">
    <div className="flex min-w-0 flex-[3_1_28rem] items-start gap-3.5">
      ...
      <div className="min-w-0">
        <div className="flex flex-wrap items-center gap-2">
          <h2 className="font-serif text-lg font-bold leading-tight text-ink"><a href={route} onClick={(event) => { event.preventDefault(); onNavigate(route); }} className="break-words transition-colors hover:text-accent-hover before:absolute before:inset-0 before:content-['']">{title}</a></h2>
          {badge || <span className="text-[10px] font-mono font-bold uppercase tracking-wide text-accent">{eyebrow}</span>}
    ...
    <span className="relative z-[1] inline-flex min-h-9 shrink-0 items-center gap-1 font-mono text-xs font-semibold text-accent-hover group-hover:underline">Inspect Member Record<ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-1" aria-hidden="true" /></span>
  </article>;
}
