...
export function RepositoryCollectionTable({ records, onNavigate }: { records: CatalogRecord[]; onNavigate: Navigate }) {
  return <>
    ...
    <div className="hidden rounded-[4px] border border-[var(--color-border-soft)] bg-white lg:block">
      <table className="w-full table-fixed border-collapse text-left" aria-label="Repository catalog records">
        <thead className="bg-[var(--color-surface)] text-[9px] font-mono uppercase tracking-wide text-ink-muted"><tr><th className="w-[29%] px-3 py-3">Repository</th><th className="w-[13%] px-2 py-3">Stars</th><th className="w-[10%] px-2 py-3">Forks</th><th className="w-[14%] px-2 py-3">30-day commits</th><th className="w-[9%] px-2 py-3">Packages</th><th className="hidden w-[15%] px-2 py-3 xl:table-cell">Governance</th><th className="w-[10%] px-3 py-3 text-right">Action</th></tr></thead>
        <tbody className="divide-y divide-[var(--color-border-soft)]">{records.map((record) => { const metrics = valueRecord(record.metrics); const history = valueRecord(record.history); return <tr key={record.id} className="group hover:bg-[color-mix(in_srgb,var(--color-accent)_3%,white)]"><td className="px-3 py-3 align-top"><div className="flex gap-2.5"><span className="mt-0.5 text-emerald-700"><Code2 className="h-4 w-4" /></span><div className="min-w-0"><RecordLink route={String(record.route)} onNavigate={onNavigate}>{String(record.owner)}/{String(record.name)}</RecordLink><p className="mt-1 line-clamp-1 text-[10px] text-ink-muted">{recordDescription(record)}</p></div></div></td><td className="px-2 py-3"><strong className="block text-xs tabular-nums text-ink">{compactNumber(Number(metrics.stars || 0))}</strong><MiniLine points={valueRecords(history.stars)} label="Stars" /></td><td className="px-2 py-3"><span className="inline-flex items-center gap-1 text-xs font-semibold tabular-nums"><GitFork className="h-3 w-3 text-violet-600" />{compactNumber(Number(metrics.forks || 0))}</span></td><td className="px-2 py-3"><strong className="block text-xs tabular-nums">{compactNumber(Number(metrics.commits || 0))}</strong><MiniBars points={valueRecords(record.commit_activity)} /></td><td className="px-2 py-3"><span className="inline-flex items-center gap-1 text-xs font-semibold"><Package className="h-3 w-3 text-orange-600" />{Number(record.package_count || 0).toLocaleString()}</span></td><td className="hidden px-2 py-3 xl:table-cell">{record.ssot_governed ? <CatalogPill tone="accent" Icon={ShieldCheck}>Governed</CatalogPill> : <span className="text-[10px] text-ink-muted">Not observed</span>}</td><td className="px-3 py-3 text-right"><RecordLink route={String(record.route)} onNavigate={onNavigate}><ArrowRight className="ml-auto h-4 w-4 text-accent transition-transform group-hover:translate-x-0.5" /></RecordLink></td></tr>; })}</tbody>
      </table>
    </div>
  </>;
}
...
function PackageMobileCard({ record, onNavigate }: { record: CatalogRecord; onNavigate: Navigate }) {
  ...
  return <article className="space-y-3 rounded-[4px] border border-[var(--color-border-soft)] bg-white p-4"><div className="flex items-start gap-3"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-[3px] border border-orange-200 bg-orange-50 text-orange-700"><Box className="h-4 w-4" /></span><div className="min-w-0 flex-1"><RecordLink route={String(record.route)} onNavigate={onNavigate}>{String(record.name)}</RecordLink><p className="mt-1 text-[10px] font-mono text-ink-muted">{humanLabel(String(record.ecosystem || "unknown"))} · {repository.owner ? `${String(repository.owner)}/${String(repository.name)}` : "Repository not linked"}</p></div><PublicationBadge record={record} /></div><dl className="flex flex-wrap gap-x-6 gap-y-2 text-[10px] font-mono"><div><dt className="text-ink-muted">Latest</dt><dd className="font-bold text-ink">{String(record.latest_version || "Not recorded")}</dd></div><div><dt className="text-ink-muted">Dependencies</dt><dd className="font-bold text-ink">{Number(record.dependency_count || 0).toLocaleString()}</dd></div><div><dt className="text-ink-muted">License</dt><dd className="font-bold text-ink">{String(record.license_expression || "Not observed")}</dd></div></dl></article>;
}
...
export function PackageCollectionTable({ records, onNavigate }: { records: CatalogRecord[]; onNavigate: Navigate }) {
  return <>
    ...
    <div className="hidden rounded-[4px] border border-[var(--color-border-soft)] bg-white lg:block">
      <table className="w-full table-fixed border-collapse text-left" aria-label="Package catalog records"><thead className="bg-[var(--color-surface)] text-[9px] font-mono uppercase tracking-wide text-ink-muted"><tr><th className="w-[22%] px-3 py-3">Package name</th><th className="w-[8%] px-2 py-3">Ecosystem</th><th className="w-[19%] px-2 py-3">Owning repository</th><th className="w-[14%] px-2 py-3">Publication</th><th className="w-[10%] px-2 py-3">Latest</th><th className="w-[9%] px-2 py-3">Dependencies</th><th className="hidden w-[9%] px-2 py-3 xl:table-cell">Dependents</th><th className="hidden w-[7%] px-2 py-3 xl:table-cell">License</th><th className="w-[2%] px-2 py-3"><span className="sr-only">Action</span></th></tr></thead><tbody className="divide-y divide-[var(--color-border-soft)]">{records.map((record) => { const repository = valueRecords(record.repositories)[0] || {}; return <tr key={record.id} className="group hover:bg-[color-mix(in_srgb,var(--color-accent)_3%,white)]"><td className="px-3 py-3"><div className="flex items-center gap-2"><Package className="h-3.5 w-3.5 shrink-0 text-orange-600" /><RecordLink route={String(record.route)} onNavigate={onNavigate}>{String(record.name)}</RecordLink></div></td><td className="px-2 py-3"><span className="rounded-[3px] border border-[var(--color-border-soft)] bg-[var(--color-surface)] px-1.5 py-1 text-[9px] font-mono">{String(record.ecosystem || "unknown")}</span></td><td className="break-words px-2 py-3 text-[10px] font-mono text-accent">{repository.owner ? `${String(repository.owner)}/${String(repository.name)}` : "Not linked"}</td><td className="px-2 py-3"><PublicationBadge record={record} /></td><td className="px-2 py-3 text-xs font-semibold tabular-nums">{String(record.latest_version || "Not recorded")}</td><td className="px-2 py-3 text-xs tabular-nums">{Number(record.dependency_count || 0).toLocaleString()}</td><td className="hidden px-2 py-3 text-xs font-semibold tabular-nums text-accent xl:table-cell">{Number(record.dependent_count || 0).toLocaleString()}</td><td className="hidden break-words px-2 py-3 text-[10px] font-mono text-emerald-700 xl:table-cell">{String(record.license_expression || "Not observed")}</td><td className="px-2 py-3"><RecordLink route={String(record.route)} onNavigate={onNavigate}><ArrowRight className="h-4 w-4 text-accent transition-transform group-hover:translate-x-0.5" /></RecordLink></td></tr>; })}</tbody></table>
    </div>
  </>;
}
