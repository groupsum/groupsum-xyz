  ...
  return <div className="space-y-6 border-b border-[#E5E3DC] pb-6">
    <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
      ...
      <div className="flex flex-col items-start space-y-2 font-mono text-xs md:items-end">
        <div className="inline-flex items-center space-x-1.5 rounded-md border border-[#E5E3DC] bg-[#F4F3EF] px-2.5 py-1 text-[#5C635E]">
  ...
  return <div className="mx-auto w-full max-w-7xl space-y-6 px-4 py-8 sm:px-6 lg:px-8">
    ...
    <div className="space-y-3 py-4">
      <div className="flex flex-wrap items-center gap-3 rounded-xl border border-[#E5E3DC] bg-[#FAF9F6] p-3">
        <label className="relative min-w-[200px] flex-1">
          <span className="sr-only">Search collection</span><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[#5C635E]" />
          ...
          {search && <button type="button" onClick={() => setSearch("")} aria-label="Clear search" className="absolute right-2.5 top-1/2 -translate-y-1/2 text-[#5C635E] hover:text-[#1F2421]"><X className="h-3.5 w-3.5" /></button>}
        </label>
        {!organization && <select aria-label="Filter by owner or organization" value={owner} onChange={(event) => setOwner(event.target.value)} className="min-h-9 rounded-lg border border-[#E5E3DC] bg-white px-3 py-2 font-mono text-xs text-[#1F2421] focus:outline-none focus:ring-2 focus:ring-[#1A73E8]">
          <option value="">All Owners / Orgs</option>{owners.map((value) => <option key={value} value={value}>{organizationNames[value] || value}</option>)}
        </select>}
        <select aria-label="Filter by maturity" value={maturity} onChange={(event) => setMaturity(event.target.value)} className="min-h-9 rounded-lg border border-[#E5E3DC] bg-white px-3 py-2 font-mono text-xs text-[#1F2421] focus:outline-none focus:ring-2 focus:ring-[#1A73E8]">
          <option value="">All Maturity States</option>{maturities.map((value) => <option key={value} value={value}>{value.replace(/[-_]+/g, " ")}</option>)}
        </select>
        <div className="ml-auto flex items-center space-x-1"><ArrowUpDown className="h-3.5 w-3.5 text-[#5C635E]" /><select aria-label="Sort collection" value={sortBy} onChange={(event) => setSortBy(event.target.value)} className="min-h-9 rounded-lg border border-[#E5E3DC] bg-white px-3 py-2 font-mono text-xs text-[#1F2421] focus:outline-none focus:ring-2 focus:ring-[#1A73E8]"><option value="name">Sort: Name (A-Z)</option><option value="activity">Sort: Most Activity</option><option value="recent">Sort: Featured First</option></select></div>
      </div>
      {hasFilters && <div className="flex flex-wrap items-center gap-2 pt-1 font-mono text-xs"><span className="font-medium text-[#5C635E]">Active Filters:</span>{search && <span className="rounded-full border border-[#E5E3DC] bg-white px-2.5 py-1 text-[#1F2421]">Query: <strong>{search}</strong></span>}{owner && !organization && <span className="rounded-full border border-[#E5E3DC] bg-white px-2.5 py-1 text-[#1F2421]">Owner: <strong>{organizationNames[owner] || owner}</strong></span>}{maturity && <span className="rounded-full border border-[#E5E3DC] bg-white px-2.5 py-1 text-[#1F2421]">Maturity: <strong>{maturity}</strong></span>}<button type="button" onClick={() => { setSearch(""); if (!organization) setOwner(""); setMaturity(""); }} className="ml-2 font-semibold text-[#2E6B9E] hover:underline">Clear all</button></div>}
      <div className="flex justify-between border-b border-[#E5E3DC]/60 pb-2 font-mono text-xs text-[#5C635E]"><span>Showing <strong className="font-bold text-[#1F2421]">{filtered.length}</strong> matching records</span><span className="hidden text-[#5C635E] sm:inline">Filtered projection</span></div>
    </div>
    <div className="space-y-3">
      {filtered.map((record) => {
        ...
        return <div key={record.id} role="link" tabIndex={0} onClick={() => onNavigate(route)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); onNavigate(route); } }} className="product-portfolio-card catalog-density-card group flex cursor-pointer flex-col justify-between gap-4 rounded-xl border border-[#E5E3DC] bg-white p-5 shadow-sm transition-all duration-200 hover:border-[#1A73E8] hover:bg-[#FAF9F6] md:flex-row md:items-center">
          <div className="flex min-w-0 flex-1 items-start gap-3"><div className="shrink-0 rounded-[3px] border border-[#E5E3DC] bg-[#F4F3EF] p-2 transition-colors group-hover:bg-white">{record.recordType === "portfolio" ? <Layers className="h-4 w-4 text-[#5B4699]" /> : <Box className="h-4 w-4 text-[#2E6B9E]" />}</div><div className="min-w-0 flex-1 space-y-1"><div className="flex flex-wrap items-center gap-2"><span className="truncate font-serif text-base font-bold text-[#1F2421] transition-colors group-hover:text-[#1A73E8]">{record.title}</span>{maturityBadge(record.maturity)}</div><p className="line-clamp-2 text-xs leading-5 text-[#5C635E] md:line-clamp-1">{record.summary}</p><div className="flex flex-wrap items-center gap-x-2.5 gap-y-1 font-mono text-[11px] text-[#5C635E]"><span className="inline-flex items-center gap-1"><Building2 className="h-3 w-3" /><span>{organizationNames[record.organization] || record.organization}</span></span><span className="text-[#D5D8D6]">•</span><span className="inline-flex items-center gap-1 font-medium text-[#5C635E]"><Cpu className="h-3 w-3" />{visibleTechnologies.length ? visibleTechnologies.join(" · ") : "Tech not reported"}</span><span className="text-[#D5D8D6]">•</span><span className="inline-flex items-center gap-1 text-[#5B4699]"><GitBranch className="h-3 w-3" />{record.repositoryCount} repos</span><span className="text-[#D5D8D6]">•</span><span className="font-semibold text-[#2E6B9E]">{record.packageCount} packages</span></div></div></div>
          <div className="flex shrink-0 items-center justify-end border-t border-[#E5E3DC]/60 pt-2 md:border-t-0 md:pt-0"><span className="inline-flex items-center gap-1 font-mono text-xs font-semibold text-[#1A73E8] group-hover:underline">Inspect Member Record<ChevronRight className="h-4 w-4 transition-transform group-hover:translate-x-0.5" /></span></div>
        </div>;
      })}
      {filtered.length === 0 && <div className="rounded-xl border border-[#E5E3DC] bg-white p-8 text-center font-mono text-xs text-[#5C635E]">No records match the active filter criteria. Try adjusting search terms or clearing filters.</div>}
    </div>
  </div>;
}
