  ...
  return <ul className="border-y border-[var(--color-border-soft)] divide-y divide-[var(--color-border-soft)]">
    {registries.map((registry) => {
      ...
      return <li key={registry.repository_id} className="py-5 space-y-4">
        ...
        {claims.length > 0 && <div><h4 className="text-[10px] font-mono uppercase text-ink font-semibold mb-2">Registry claims</h4><ul className="divide-y divide-[var(--color-border-soft)] border-y border-[var(--color-border-soft)]">{claims.slice(0, 20).map((claim) => <li key={claim.id} className="py-2 text-xs text-ink"><span className="font-mono text-accent">{claim.id}</span>{claim.statement || claim.title || claim.name ? <span className="text-ink-muted"> Â· {claim.statement || claim.title || claim.name}</span> : null}</li>)}</ul></div>}
        {evidence.length > 0 && <div><h4 className="text-[10px] font-mono uppercase text-ink font-semibold mb-2">Registry evidence</h4><ul className="divide-y divide-[var(--color-border-soft)] border-y border-[var(--color-border-soft)]">{evidence.slice(0, 20).map((item) => <li key={item.id} className="py-2 text-xs text-ink"><span className="font-mono text-accent">{item.id}</span>{item.title || item.name ? <span className="text-ink-muted"> Â· {item.title || item.name}</span> : null}</li>)}</ul></div>}
...
export function ProductRecordPage({
  ...
  if (!entity && !pageModel) {
    ...
    return <div className="max-w-3xl mx-auto px-4 py-20 space-y-4"><h1 className="font-serif text-3xl font-bold text-ink">{loading ? "Loading portfolio inventory" : "Portfolio record unavailable"}</h1><p className="text-sm text-ink-muted">{loading ? "Loading the durable catalog recordâ€¦" : "This public product or portfolio record could not be found."}</p><button onClick={() => onNavigate(recordType === "product" ? "/products" : "/portfolio")} className="text-xs font-mono text-accent hover:underline">Return to {recordType === "product" ? "products" : "portfolio"}</button></div>;
  }
      ...
      <div className="grid grid-cols-1 items-start gap-8 lg:grid-cols-12">
        ...
        <div className="min-w-0 space-y-8 lg:order-1 lg:col-span-8">
          ...
          <ProductSection id="implementation" title="Public implementation inventory" intro="Catalog observations strengthen this product record without replacing reviewed product positioning or becoming SSOT evidence.">
            {catalogState === "loading" && <p className="text-sm text-ink-muted" role="status">Loading repository and package inventoryâ€¦</p>}
          ...
          <ProductSection id="packages" title="Packages" intro="Public packages attached through implementation, website, or documentation repositories. Their role is shown explicitly.">
            ...
            </div> : <p className="text-sm text-ink-muted">{catalogState === "loading" ? "Loading packagesâ€¦" : "No public package records are attached to this product."}</p>}
          </ProductSection>
          ...
          <ProductSection id="releases" title="Release activity by owner" intro="This product member summarizes release-owning relatives. Repository records provide aggregate analytics; package records provide their comprehensive local timelines.">
            {packageReleaseGroups.length > 0 || repositoryReleaseGroups.length > 0 ? <ul className="divide-y divide-[var(--color-border-soft)]">
              {packageReleaseGroups.map((pkg) => { const path = `/catalog/packages/${pkg.ecosystem}/${pkg.route_key}`; const latest = pkg.releases[0]; return <li key={pkg.id} className="flex flex-wrap items-center justify-between gap-4 py-4"><div><span className="text-[10px] font-mono uppercase text-accent">{ecosystemLabel(pkg.ecosystem)} package</span><h3 className="font-serif text-lg font-bold text-ink">{pkg.name}</h3><p className="text-xs text-ink-muted">{pkg.release_count.toLocaleString()} releases{latest ? ` Â· latest ${latest.version}` : ""}</p></div><a href={path} onClick={(event) => { event.preventDefault(); onNavigate(path); }} className="inline-flex min-h-11 items-center gap-1 text-xs font-mono font-semibold text-accent hover:underline">Full package timeline <ArrowRight className="h-3.5 w-3.5" /></a></li>; })}
              {repositoryReleaseGroups.map((repository) => { const path = `/catalog/repositories/${repository.owner}/${repository.name}`; const latest = repository.releases[0]; return <li key={repository.id} className="flex flex-wrap items-center justify-between gap-4 py-4"><div><span className="text-[10px] font-mono uppercase text-accent">Repository</span><h3 className="font-serif text-lg font-bold text-ink">{repository.owner}/{repository.name}</h3><p className="text-xs text-ink-muted">{repository.release_count.toLocaleString()} GitHub releases{latest ? ` Â· latest ${latest.version}` : ""}</p></div><a href={path} onClick={(event) => { event.preventDefault(); onNavigate(path); }} className="inline-flex min-h-11 items-center gap-1 text-xs font-mono font-semibold text-accent hover:underline">Repository release analytics <ArrowRight className="h-3.5 w-3.5" /></a></li>; })}
          ...
          <ProductSection id="observations" title="Source observations and limitations" intro="These inventory and editorial source checks are not SSOT evidence records.">
            <div className="space-y-6">
              <div><h3 className="text-xs font-mono uppercase text-ink font-semibold mb-2">Observed sources</h3><ul className="border-y border-[var(--color-border-soft)] divide-y divide-[var(--color-border-soft)]">{observationRows.map((item, index) => <li key={`${item.label}-${index}`} className="py-3 text-sm text-ink"><span className="font-medium">{item.url ? <a href={item.url} target="_blank" rel="noreferrer" className="text-accent hover:underline">{item.label}</a> : item.label}<span className="text-ink-muted"> Â· observed {item.checkedAt}</span></span></li>)}</ul></div>
