...
const ownerOrganization = (record) => record.organizationName ? { "@type": "Organization", "@id": record.organizationId || id(record.organizationUrl || ROOT, "organization"), name: record.organizationName, ...(record.organizationUrl ? { url: record.organizationUrl } : {}) } : organization;
...
const graph = (record) => {
  ...
  const owner = ownerOrganization(record);
  const nodes = [organization, ...(owner["@id"] !== organization["@id"] ? [owner] : []), { "@type": "WebSite", "@id": id(ROOT, "website"), url: `${ROOT}/`, name: "Groupsum", publisher: { "@id": id(ROOT, "organization") } }, page, breadcrumb(record)];
  if (["products", "portfolio", "solutions", "services", "insights", "catalog-collection"].includes(schemaFamily)) nodes.push({ "@type": "ItemList", "@id": id(record.url, "items"), name: record.title, ...(record.itemCount ? { numberOfItems: record.itemCount } : {}), itemListElement: [] });
  if (["catalog", "catalog-collection"].includes(schemaFamily)) nodes.push({ "@type": "DataCatalog", "@id": id(record.url, "catalog"), name: record.title, description: record.description, url: record.url, publisher: { "@id": id(ROOT, "organization") } });
  ...
  if (["product-detail", "portfolio-detail"].includes(schemaFamily)) {
    const application = softwareApplicationNode({ id: stableId(record.url, "software"), url: record.url, name: record.title.replace(/ \|.*$/, ""), description: record.description, applicationCategory: "DeveloperApplication", operatingSystem: "Cross-platform" });
    application.provider = { "@id": owner["@id"] };
    application.additionalProperty = [["Stars", record.stars], ["Forks", record.forks], ["Contributors", record.contributors], ["Commits", record.commits]].filter(([, value]) => Number.isFinite(Number(value))).map(([name, value]) => ({ "@type": "PropertyValue", name, value }));
